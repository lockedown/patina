...
- **No "recomputing" feedback during live audition.** `akz_realtime_player_is_ready` exists in the C API and is unused by the Swift/UI layer — a slow re-render currently gives no visual cue that anything is happening.
- **Save Processed saves the last offline `Process()` result, not what live audition is currently playing.** There's no "save what I'm hearing right now" path.
...
│   │   ├── AkaizerSApp.swift        Entry point + the activation-policy fix (see Status)
│   │   ├── LCDReadoutView.swift     The one retro accent -- fixed-palette phosphor-green readout
│   │   └── WaveformView.swift       Original/processed overlay, same palette as the LCD -- display only, not scrubable
...
└── Tests/
    ...
    ├── AudioTests/               XCTest — WAV/AIFF bit-exact round trip + full decode→stretch→encode→save chain, incl. transpose, PresetStoreTests.swift and PCMConversionLoudnessTests.swift
