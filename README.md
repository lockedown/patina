...
5. **Sample-rate/bandwidth front end** (`RateModel.cpp`) — closes the S900/S950 bandwidth gap above: a real anti-alias filter followed by true decimation and zero-order-hold reconstruction, computed length-neutrally. `sampleRateHz == 0` (every existing preset and default) means "no rate stage," so this shipped with zero audible change until something explicitly sets a rate.
